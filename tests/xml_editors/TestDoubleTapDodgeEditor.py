...
class TestDoubleTapDodgeEditor(unittest.TestCase):
    ...
    @patch.object(
        src.xml_editors.DoubleTapDodgeEditor.DoubleTapDodgeEditor, "disable_dodge"
    )
    @patch("src.xml_editors.Editor.ElementTree")
    @patch.object(src.xml_editors.Editor.Editor, "write")
    def test_it_may_disable_double_tap_doge(self, write, element_tree, disable_dodge):
    ...
    @patch.object(
        src.xml_editors.DoubleTapDodgeEditor.DoubleTapDodgeEditor, "enable_dodge"
    )
    @patch("src.xml_editors.Editor.ElementTree")
    @patch.object(src.xml_editors.Editor.Editor, "write")
    def test_it_may_enable_double_tap_doge(self, write, element_tree, enable_dodge):
        editor = DoubleTapDodgeEditor(config=self.config)
        editor.xpaths = {"alpha": "alpha-path", "beta": "beta-path"}
        editor.enable()
        calls = [call("alpha-path"), call("beta-path")]
